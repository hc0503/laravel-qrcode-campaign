		...
		<div class="card-body">
			<form action="{{ route('users.update', $user->id) }}" method="POST">
				...
				<fieldset class="form-group">
					...
					<input name="name" type="text" class="form-control" id="name" placeholder="@lang('locale.Name')" value="{{ $user->name }}">
					<span class="danger">{{ $errors->first('name') }}</span>
				</fieldset>

				<fieldset class="form-group">
					<label for="surname">@lang('locale.Surname')*</label>
					<input name="surname" type="text" class="form-control" id="surname" placeholder="@lang('locale.Surname')" value="{{ $user->surname }}">
					<span class="danger">{{ $errors->first('surname') }}</span>
				</fieldset>

				<fieldset class="form-group">
					<label for="email">@lang('locale.Email')*</label>
					<input name="email" type="email" class="form-control" id="email" placeholder="@lang('locale.Email')" value="{{ $user->email }}">
				...
				<fieldset class="form-group">
					<label for="name">@lang('locale.role.title')*</label>
					<select name="roles[]" class="select2 form-control" multiple="multiple">
						@php
						$arrRoles = $user->roles->pluck("name")->toArray();
						@endphp
						@foreach ($roles as $role)
						<option value="{{ $role->name }}" {{ in_array($role->name, $arrRoles) ? "selected" : "" }} >{{ $role->name }}</option>
						@endforeach
					</select>
				...
				<div class="text-right">
					<button class="btn btn-primary">@lang('locale.user.update')</button>
				</div>
			</form>
		</div>
		</div>
